...
// Removed Chinese language constants and functions - now supporting Indian languages only

function getElementAnnotation(number, lang) {
  // No annotations for Indian languages yet - returning empty for now
  return "";
}
...
// Removed Chinese language constants - now supporting Indian languages only

// Removed Chinese language support - keeping data structure for now
const COMMON_ION_ZH = {
...
export function showModal(element) {
  ...
  initElementTutorial();
  document.title = `VED-AI Visualizer - ${localizeElementName(element)}`;
...
export function initModalUI() {
  ...
  function closeElementModal() {
    ...
    document.title = "VED-AI Visualizer";
    clearHeadlineResizeHandler();
    cleanup3D(true);
    atomContainer.classList.remove("visible");
    resetModalUI();
