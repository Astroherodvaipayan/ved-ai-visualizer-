...
export function ensureThreeLibLoaded() {
  ...
  threeLoadPromise = new Promise((resolve, reject) => {
    const existing = document.querySelector('script[data-vedai-three="1"]');
    if (existing) {
...
export function updateAtomStructure(element) {
  ...
  if (atomicNumber > 1) {
    ...
    if (atomicNumber === 11 && window._vedaiNaMode === "cloud") lightIntensity = 5.0;
...
export function animateAtom() {
  ...
  const isPaused = window._vedaiAnimPaused || false;
  const speedMul = (typeof window._vedaiAnimSpeed === "number") ? window._vedaiAnimSpeed : 0.6;
  ...
  const isEdu = isNa && window._vedaiNaMode === "educational";
